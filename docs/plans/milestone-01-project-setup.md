...
- [x] Đã có source file riêng trong `Persistence`.
- [ ] Chưa có `.xcodeproj` thật; deferred sang milestone 08 để tạo project/archive target thật.
...
- [x] Bổ sung `Sources/MLingoCore/Persistence/PersistencePolicy.swift` để chốt policy persistence của MVP.
- [x] Tạo logging utility bằng `OSLog` với các category: `audio`, `whisper`, `translation`, `overlay`, `settings`, `pipeline`.
- [x] Gắn logging tối thiểu vào audio capture, translation và subtitle pipeline.
- [ ] Nếu bắt buộc cần Xcode project, tạo project thật trong Xcode hoặc bằng tool được phê duyệt ở milestone 08.
- [x] Không tạo `.xcodeproj` bằng tay nếu không verify được scheme.
- [ ] Xcode project thật phải tham chiếu đúng source hiện có, không duplicate file.

## Tiêu chí hoàn thành

- [x] `rtk proxy swift test` pass.
- [x] `rtk proxy swift build` pass và được ghi nhận.
